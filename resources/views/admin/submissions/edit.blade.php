    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.submissions.update", [$submission->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('assignment') ? 'is-invalid' : '' }}" name="assignment_id" id="assignment_id">
                    @foreach($assignments as $id => $entry)
                        <option value="{{ $id }}" {{ (old('assignment_id') ? old('assignment_id') : $submission->assignment->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('student_efk') ? 'is-invalid' : '' }}" type="number" name="student_efk" id="student_efk" value="{{ old('student_efk', $submission->student_efk) }}" step="1">
